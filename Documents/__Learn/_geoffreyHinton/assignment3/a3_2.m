function a3_2(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
...
function ret = loss(model, data, wd_coefficient)
  ...
  % data.targets is a matrix of size (10,<number of data cases>)
  ...
  classification_loss = -mean(sum(log_class_prob .* data.targets, 1)); % select the cross entropy right log class probability using that sum; then take the mean over all data cases.
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  error_deriv = class_out - data.targets;  %Error derivate w.r.t. zj size (10,<number of data cases>    
