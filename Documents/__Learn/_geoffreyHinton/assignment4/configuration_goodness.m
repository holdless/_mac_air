...
G = mean(diag(hidden_state'*rbm_w*visible_state)); % diag means do the average on matched(pair) cases
% G= sum(sum(visible_state*hidden_state'.*rbm_w'))/size(visible_state,2);
end
