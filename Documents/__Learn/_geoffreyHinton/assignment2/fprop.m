...
[numwords, batchsize] = size(input_batch);              % input_batch [3x100]
[vocab_size, numhid1] = size(word_embedding_weights);   % [250(words in database)x50(word distribution featues neurons for each word)]
numhid2 = size(embed_to_hid_weights, 2);                % hidden layer neurons: [200]

%% COMPUTE STATE OF WORD EMBEDDING LAYER.
% Look up the inputs word indices in the word_embedding_weights matrix.
embedding_layer_state = reshape(...
  word_embedding_weights(reshape(input_batch, 1, []),:)',...
  numhid1 * numwords, []);                  % 2nd layer state (1st layer is input layer): [150(3*50)x100]   
                                            % 1st-2nd weight: word_embedding_weights [250x50]

%% COMPUTE STATE OF HIDDEN LAYER.
% Compute inputs to hidden units.
inputs_to_hidden_units = embed_to_hid_weights' * embedding_layer_state + ...
  repmat(hid_bias, 1, batchsize);           % 3rd layer pre-state [200x100]   
                                            % 2nd-3rd weight: [150x200]
...
% (c) 
hidden_layer_state = 1 ./ (1 + exp(-inputs_to_hidden_units));   % 3rd layer post-state [200x100]
% (d) hidden_layer_state = -1 ./ (1 + exp(-inputs_to_hidden_units));

%% COMPUTE STATE OF OUTPUT LAYER.
% Compute inputs to softmax.
% FILL IN CODE. Replace the line below by one of the options.
inputs_to_softmax = zeros(vocab_size, batchsize);   % 4th pre-state [200x100]
                                                    % 3rd-4th weight [200x250]
% Options
% (a) 
