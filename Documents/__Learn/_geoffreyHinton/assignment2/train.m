...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    error_deriv = output_layer_state - expanded_target_batch; % [250x100] : (y_j - t_j)
    ...
    % BACK PROPAGATE.
    %% OUTPUT LAYER.
    hid_to_output_weights_gradient =  hidden_layer_state * error_deriv'; % dC/dz_j = y_j - t_j
    
    output_bias_gradient = sum(error_deriv, 2); % column sum
...
%     back_propagated_deriv_2 = zeros(numhid2, batchsize);
    back_propagated_deriv_2 = zeros(numhid1, batchsize);
    % Options
    % (a) 
    ...
    word_embedding_weights_delta = ...
      ...
      word_embedding_weights_gradient ./ batchsize; % <--- take average. the graident matrix is a sum.
